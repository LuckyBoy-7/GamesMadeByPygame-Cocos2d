...
def run_simulator():
    ...
    # 构建模拟人员（下面会给出函数实现）
    grids = create_viruses(settingInfo, screen, persons)
    ...
    while True:
        ...
        for person in persons.sprites():
            ...
            person.update_position(settingInfo, persons)
        for person in persons.sprites():
            # 注意，该处会调用每个模拟人员的update函数，我之前自己实现的
            person.update_color(settingInfo, persons)
        # 绘制按钮
...
# 创建模拟人员函数
def create_viruses(settingInfo, screen, persons) -> Dict[tuple, set]:
    dic = {(x, y): set() for x in range(settingInfo.columns) for y in range(settingInfo.rows)}
    while True:

        person = Person(settingInfo, screen, dic)
        person.draw_person()
        persons.add(person)
        dic[person.pos].add(person)
