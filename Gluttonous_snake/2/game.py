...
from snake import Snake, AppleProp, Pos
...
class GameLayer(Layer):
    ...
    def init(self):
        ...
        self.snake_bodies = deque()
        self.snake_bodies.append(Snake(pos=Pos(0, 0), game_layer=self, config=self.config))
    ...
    def check_valid(self, pos):
        # 碰到身体或撞墙游戏失败
        if pos[0] < 0 or pos[0] >= self.config.columns or \
                pos[1] < 0 or pos[1] >= self.config.rows:
            return False
        if isinstance(self.grids[pos], Snake):
            return False
        return True

    def move_snake(self):
        self.move_elapsed += 1
        if self.move_elapsed > 45:
            self.move_elapsed = 0
            self._move_snake()

    def _move_snake(self):
        pos = Pos(*self.direction[self.snake_direction]) + self.snake_bodies[-1]
        if self.check_valid(*pos):
            self.snake_bodies.append(Snake(pos=pos, game_layer=self, config=self.config))
    ...
    def game_over(self):
        ...
        self.create_label(text="Game Over~",
                          ...
                          size=100,
                          color=(0, 0, 0, 255))
        self.create_label(text="Press 'r' to repos",
    ...
    def repos(self):
        for _, child in self.children:
            if isinstance(child, (Line, Label, AppleProp)):
                self.remove(child)
        self.init()
        self.schedule(self.update)
