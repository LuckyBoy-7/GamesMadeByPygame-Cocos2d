...
class ScoreBoard:
    def __init__(self, *args):
        ...
        try:
            self.font = pygame.font.Font("../res/font/font.ttf", 35)
        except Exception as e:
            print(e)
            self.font = pygame.font.SysFont(None, 50)
    def draw(self, display_surface):
        score_image = self.font.render(str(self.stats.score),
                                       True, self.score_color)
...
class PauseResume:
    ...
    def update_click(self):
        if (self.state == PauseResume.PAUSE_NORMAL or
...
class EndPrompt:
    def __init__(self, *args):
        ...
        self.score_text_image = self.font.render("Score:",
                                                 True, self.score_color)
    ...
    def update_score_num(self):
        self.score_num_image = self.font.render(str(self.stats.score),
                                       True, self.score_color)
        self.score_num_rect = self.score_num_image.get_rect()
        self.score_num_rect.centerx = self.border_rect.centerx
        self.score_num_rect.centery = self.border_rect.y + 80

    def draw(self, display_surface):
        pygame.draw.rect(display_surface, self.border_color, self.border_rect, 3)

        display_surface.blit(self.score_text_image, (self.score_text_left,
                                                     self.score_text_top))
